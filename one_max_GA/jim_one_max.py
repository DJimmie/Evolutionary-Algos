...
def populationSum(fitness):
    """The population sum for the Fitness Score"""
    s=sum(fitness)
    print(f'\nPopulation Sum: {s}\n')
...
def plot_hof(hof):
    ...
    plt.plot(x,y)
    plt.title(f'POPULATION SUM\n(Target:{target})\nMax value:{max(y)}',fontdict=font,)
    plt.ylabel(f'population sum (target:{target})')
    plt.xlabel('Generation')
    plt.ylim(min(y),target)
    plt.grid()
    plt.show()
