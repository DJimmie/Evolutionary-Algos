...
def fitness_score(x):
    ...
    xmin=XMIN
    xmax=XMAX
    r=xmin+((xmax-xmin)/((2**NUM_BITS)-1))*d  #------>RADIUS
    print(f'Scaled Decision Variable ({VARIABLE_NAME}): {round(r,2)}')

    ## Get Area of the circle as the score

    # if (x>.5*WIDTH_OF_SQUARE): ------> *contrains the radius to within 1/2 the width os square
    # A=0

    ## Scenerio: Find the radius of a circle that results in the specified remaining area (TARGET_AREA) in the square
    area_of_square=WIDTH_OF_SQUARE**2
    A=math.pi*r**2
    actual_remaining_area=area_of_square-A
    print(f'remaining area:{actual_remaining_area}\n{actual_remaining_area>1000}')
    TARGET_AREA=8585  # desired area remaining in the square
    if (actual_remaining_area<TARGET_AREA):
        A=0
...
def plot_hof(hof):
    ...
    radius=[math.sqrt((i/math.pi)) for i in y]
    ...
    axs[0].plot(x, y)
    axs[0].set_title(f'POPULATION AVERAGE AREA\nMax value:{round(max(y))}',fontdict=font)
    axs[0].grid()
    axs[0].set_xlabel('Generation')
    axs[0].set_ylabel('Area of Circle')
    axs[1].scatter(x, radius)
    axs[1].set_title(f'POPULATION AVERAGE RADIUS\nMax value:{max(radius):8.2f}',fontdict=font)
    axs[1].grid()
    axs[1].set_xlabel('Generation')
    axs[1].set_ylabel('Radius of Circle')
    plt.show()
